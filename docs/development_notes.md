...
- [-] プロンプトから「埋め込み設定」を抽出するロジックを実装する (正規表現とJSONパースを使用)
- [ ] 抽出した設定を画像生成パラメータに適用するロジックを実装する
- [ ] UIにトグルボタンを追加し、Extensionの有効/無効を切り替える機能を実装する (WebUIの拡張機能設定に配置)
- [ ] 他のユーザースクリプトとの併用を考慮した設計を行う
- [ ] テストコードを作成し、機能が正しく動作することを確認する
